...
class DualBranchFusion(nn.Module):
    def __init__(self, fusion="cross", n_static=14, dropout=0.3, heads=8,
                 sat_kwargs=None, weather_kwargs=None):
        super().__init__()
        ...
        d_weather = weather_kwargs["d_model"]
        ...
        if fusion == "cross":
            self.q_proj = nn.Linear(d_weather, d_sat)      # bridge 192 -> 256
            self.attn = nn.MultiheadAttention(d_sat, heads, dropout=dropout,
                                              batch_first=True)
            fused_dim = d_sat
        else:  # concat
            fused_dim = d_sat + d_weather
    ...
    def forward(self, batch):
        ...
        if self.fusion == "cross":
            ...
            fused = ctx.mean(dim=1)                          # pool over days
        else:
